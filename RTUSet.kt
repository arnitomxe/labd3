 ...
 */
infix fun Set.contains(elem: Int): Boolean = this(elem)//true
/**
 ...
 */
infix fun Set.union(set: Set): Set = { singletonSet(it).contains(it)}//{ true }//{ it == it}
    //{ contains(it) }     forAll tests strādā lietojot šīm līnijām, bet union tests vairs nestrādā
    //{this.contains(it)}

/**
 * Intersection of the two sets
 */
infix fun Set.intersect(set: Set): Set = set
    //{ this==set }

/**
 * Difference of two given sets
 */
infix fun Set.diff(set: Set): Set = { intersect(set).contains(it).not()}
...
infix fun Set.exists(predicate: (Int) -> Boolean): Boolean = TODO()//when{predicate (it)->true else -> false}

fun Set.map(transform: (Int) -> Int): Set = {singletonSet(it).contains(it)}
